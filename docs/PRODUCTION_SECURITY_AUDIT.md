...
**Finding CRIT-05 (AES S-Box Cache Timing)**: The AES-256 implementation at `kernel_crypto.c:364` uses a traditional 256-entry S-box lookup table. On systems with a cache (all modern x86), the table access pattern reveals the key byte ⊕ plaintext byte relationship. This is the canonical AES cache-timing side channel. **Impact**: CRITICAL if the attacker can run code on the same CPU (co-located VM, speculative execution). **Mitigation**: ChaCha20-Poly1305 is available as an alternative and is constant-time. The default AEAD for vault data is AES-256-GCM-SIV (due to hardware AES acceleration on modern CPUs), but the S-box leakage is present. **Recommendation**: Switch to ChaCha20-Poly1305 for all encryption, or implement a bitsliced constant-time AES. For production deployments using hardware with AES-NI, the S-box is not used (AES-NI is not currently leveraged; this is a manual implementation).
...
| CRIT-05 | CRITICAL | AES S-box cache timing | OPEN — see §9 |
...
**Total findings: 19** (1 unmitigated CRITICAL, 2 OPEN HIGH, 3 OPEN MEDIUM)
...
- CRIT-05: Replace AES table-lookup S-box with bitsliced constant-time implementation, or switch default AEAD to ChaCha20-Poly1305
